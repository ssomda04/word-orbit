...
def _isolated_embedding_provider() -> Iterator[None]:
    """Keep the process-wide embedding service from leaking between tests.

    Both the settings and the built service are cached for the lifetime of the
    process. A test that points `EMBEDDING_PROVIDER` at FastText would otherwise
    hand its service to every test that ran afterwards.
    """
